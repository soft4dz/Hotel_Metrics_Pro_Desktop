...
function HotelAnalyseGridComponent({ hotels }: HotelAnalyseGridProps) {
  if (hotels.length === 0) {
    return (
      <p className="rounded-lg border border-dashed border-border/70 bg-slate-50/60 px-4 py-8 text-center text-sm text-muted-foreground">
...
const HotelCard = memo(function HotelCard({ hotel }: { hotel: HotelAnalyseRow }) {
  ...
  return (
    <article className="flex flex-col overflow-hidden rounded-xl border border-border/60 bg-white shadow-sm transition-all duration-200 hover:border-primary/20 hover:shadow-md">
      {/* Header */}
      <div className="p-4">
        ...
        <div className="mt-4">
          <div className="mb-1.5 flex justify-between text-[11px] font-medium text-muted-foreground">
            ...
            <span className="tabular-nums text-foreground">{formatPercent(hotel.tauxRealisation)}</span>
          </div>
  ...
  return (
    <div className="flex h-1.5 w-full overflow-hidden rounded-full">
      {rubriques.map((r) => (
        <div
          key={r.code}
          style={{ width: `${(r.realise / total) * 100}%`, background: rubColor(r.code) }}
...
function RubriqueRow({ row }: { row: HotelRubriqueRow }) {
  return (
    <li className="flex items-center gap-2">
      <span className="h-2 w-2 shrink-0 rounded-full" style={{ background: rubColor(row.code) }} />
      <span className="min-w-0 flex-1 truncate text-[11px] text-muted-foreground">{row.label}</span>
      <span className="shrink-0 font-mono text-[11px] tabular-nums text-foreground">{formatMoney(row.realise)}</span>
      <span className="w-8 shrink-0 text-right text-[10px] text-muted-foreground/70 tabular-nums">{row.pctTotal}%</span>
    </li>
  );
}
