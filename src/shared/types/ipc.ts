...
export interface IpcApi {
  ...
  clients: {
    ...
    crmDashboard: (hotelId?: number) => Promise<IpcResult<unknown>>;
    crmClient360: (clientId: number) => Promise<IpcResult<unknown>>;
    listCrmConsents: (clientId: number) => Promise<IpcResult<unknown[]>>;
    recordCrmConsent: (input: {clientId:number;finalite:'marketing_email'|'marketing_sms'|'marketing_whatsapp'|'profilage'|'enquete'|'portail';accorde:boolean;source:string;preuve?:string;adresseIp?:string;versionNotice?:string}) => Promise<IpcResult<unknown>>;
    listCrmSegments: () => Promise<IpcResult<unknown[]>>;
    createCrmSegment: (input: {code:string;nom:string;description?:string;criteres:{minStays?:number;minRevenue?:number;maxDaysSinceStay?:number;vip?:boolean;wilaya?:string;source?:string};dynamique?:boolean}) => Promise<IpcResult<unknown>>;
    recomputeCrmSegment: (id: number) => Promise<IpcResult<unknown>>;
    enrollLoyalty: (clientId: number) => Promise<IpcResult<unknown>>;
    moveLoyaltyPoints: (input: {clientId:number;type:'gain'|'utilisation'|'expiration'|'ajustement';points:number;motif:string;sourceType?:string;sourceId?:number;expiresAt?:string}) => Promise<IpcResult<unknown>>;
    createCrmCampaign: (input: {nom:string;segmentId:number;canal:'email'|'sms'|'whatsapp'|'multicanal';objet?:string;contenu:string;datePlanifiee?:string}) => Promise<IpcResult<unknown>>;
    prepareCrmCampaign: (id: number) => Promise<IpcResult<unknown>>;
    queueCrmMessage: (input: {clientId?:number;reservationId?:number;channel:'email'|'sms'|'whatsapp'|'portail';destinataire:string;contenu:string;provider?:string;transactional?:boolean}) => Promise<IpcResult<unknown>>;
    nextCrmMessages: (limit?: number) => Promise<IpcResult<unknown[]>>;
    completeCrmMessage: (input: {id:number;success:boolean;providerReference?:string;error?:string;delivered?:boolean}) => Promise<IpcResult<unknown>>;
    createCrmSurvey: (input: {hotelId:number;nom:string;declencheur:'checkout'|'manuel'|'reclamation';questions:unknown[]}) => Promise<IpcResult<unknown>>;
    inviteCrmSurvey: (input: {enqueteId:number;clientId?:number;reservationId?:number}) => Promise<IpcResult<unknown>>;
    submitCrmSurvey: (input: {token:string;noteNps?:number;noteGlobale?:number;commentaire?:string;reponses?:unknown}) => Promise<IpcResult<unknown>>;
    crmNpsDashboard: (hotelId: number) => Promise<IpcResult<unknown>>;
    ingestCrmReview: (input: {hotelId:number;clientId?:number;source:string;externalId?:string;auteur:string;note:number;titre?:string;commentaire?:string;publieAt?:string}) => Promise<IpcResult<unknown>>;
    listCrmReviews: (hotelId: number) => Promise<IpcResult<unknown[]>>;
    respondCrmReview: (id: number, response: string) => Promise<IpcResult<boolean>>;
    inviteCrmPortal: (clientId: number, email?: string) => Promise<IpcResult<unknown>>;
    activateCrmPortal: (input: {token:string}) => Promise<IpcResult<boolean>>;
    crmPortalOverview: (input: {clientId:number}) => Promise<IpcResult<unknown>>;
    invitePrecheckin: (reservationId: number) => Promise<IpcResult<unknown>>;
    submitPrecheckin: (input: {token:string;heureArriveePrevue?:string;adresse?:string;nationalite?:string;typeDocument?:string;numeroDocument?:string;documentPath?:string;consentementFichePolice:boolean}) => Promise<IpcResult<unknown>>;
    validatePrecheckin: (id: number, approved: boolean) => Promise<IpcResult<boolean>>;
  };
  tresorerie: {
  ...
  hebergement: {
    ...
    configureChannelConnector:(input:unknown)=>Promise<IpcResult<boolean>>;
    saveChannelMapping:(input:unknown)=>Promise<IpcResult<boolean>>;
    getDistributionAvailability:(hotelId:number,from:string,to:string)=>Promise<IpcResult<unknown[]>>;
    saveChannelRestriction:(input:unknown)=>Promise<IpcResult<boolean>>;
    queueChannelSync:(connectorId:number,from:string,to:string)=>Promise<IpcResult<number[]>>;
    nextChannelSyncJobs:(limit?:number)=>Promise<IpcResult<unknown[]>>;
    completeChannelSyncJob:(id:number,result:unknown)=>Promise<IpcResult<boolean>>;
    listChannelSyncLogs:(connectorId:number)=>Promise<IpcResult<unknown[]>>;
    ingestChannelWebhook:(input:unknown)=>Promise<IpcResult<unknown>>;
    saveBookingEngineConfig:(input:unknown)=>Promise<IpcResult<boolean>>;
    saveBookingPromo:(input:unknown)=>Promise<IpcResult<boolean>>;
    quoteDirectBooking:(input:unknown)=>Promise<IpcResult<unknown>>;
    createDirectBooking:(input:unknown)=>Promise<IpcResult<unknown>>;
    confirmDirectBookingPayment:(input:unknown)=>Promise<IpcResult<unknown>>;
    getDistributionDashboard:(hotelId:number)=>Promise<IpcResult<unknown>>;
