...
export const INTERVENTION_STATUTS = ['demandee', 'planifiee', 'en_cours', 'terminee', 'annulee'] as const;
export const INTERVENTION_TYPES = ['preventive', 'corrective', 'ameliorative'] as const;
export const INTERVENTION_PRIORITES = ['urgente', 'haute', 'normale', 'basse'] as const;

function assertHotelAccess(actorId: number, hotelId: number): void {
  if (!actorCanAccessHotel(getActorContext(actorId), hotelId)) throw new Error('Accès refusé à cette unité.');
}
...
export function listEquipements(actorId: number, hotelId: number): Equipement[] {
  assertHotelAccess(actorId, hotelId);
...
export function createEquipement(actorId: number, input: CreateEquipementInput): Equipement {
  assertHotelAccess(actorId, input.hotelId);
  ...
  const equipement = mapEquip(db.prepare(`SELECT * FROM equipements WHERE id = ?`).get(res.id) as Record<string, unknown>);
  writeAuditLog({ userId: actorId, action: 'CREATE', module: 'maintenance', description: `Équipement ${equipement.code} créé` });
  return equipement;
}

export function listInterventions(actorId: number, hotelId: number, statut?: string): Intervention[] {
  assertHotelAccess(actorId, hotelId);
  const db = getDatabase();
...
export function createIntervention(actorId: number, input: CreateInterventionInput): Intervention {
  assertHotelAccess(actorId, input.hotelId);
  const db = getDatabase();
  const statutInitial = input.datePlanifiee ? 'planifiee' : 'demandee';
  const res = db.prepare(`INSERT INTO interventions (hotel_id,equipement_id,type_intervention,titre,description,priorite,statut,technicien_id,date_planifiee,cree_par) VALUES (?,?,?,?,?,?,?,?,?,?) RETURNING id`)
    .get(input.hotelId, input.equipementId ?? null, input.typeIntervention ?? 'corrective', input.titre, input.description ?? null, input.priorite ?? 'normale', statutInitial, input.technicienId ?? null, input.datePlanifiee ?? null, actorId) as { id: number };
  const intervention = listInterventions(actorId, input.hotelId).find(i => i.id === res.id)!;
  writeAuditLog({ userId: actorId, action: 'CREATE', module: 'maintenance', description: `Intervention #${res.id} créée` });
  return intervention;
}

export function updateIntervention(actorId: number, id: number, input: UpdateInterventionInput): Intervention {
  const db = getDatabase();
  const current = db.prepare(`SELECT hotel_id, statut FROM interventions WHERE id=?`).get(id) as { hotel_id: number; statut: string } | undefined;
  if (!current) throw new Error('Intervention introuvable');
  assertHotelAccess(actorId, current.hotel_id);
  if (input.statut !== undefined && input.statut !== current.statut) {
    const transitions: Record<string, readonly string[]> = {
      demandee: ['planifiee', 'en_cours', 'annulee'],
      planifiee: ['en_cours', 'annulee'],
      en_cours: ['terminee', 'annulee'],
      terminee: [],
      annulee: [],
    };
    if (!(transitions[current.statut] ?? []).includes(input.statut)) {
      throw new Error(`Transition maintenance interdite: ${current.statut} → ${input.statut}`);
    }
  }
  ...
  writeAuditLog({ userId: actorId, action: 'UPDATE', module: 'maintenance', description: `Intervention #${id} mise à jour` });
  return mapInterv(row as Record<string, unknown>);
}

export function getMaintenanceStats(actorId: number, hotelId: number): { total: number; enCours: number; urgentes: number; coutMois: number } {
  assertHotelAccess(actorId, hotelId);
  const db = getDatabase();
