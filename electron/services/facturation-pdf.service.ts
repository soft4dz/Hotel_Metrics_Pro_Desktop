...
export async function exportFacturationPdf(actorUserId: number, factureId: number): Promise<RhExportResult> {
  ...
  const draw = (text: string, size = 10, isBold = false, x = 45) => {
    ...
    ctx.page.drawText(sanitizePdfText(text).slice(0, 100), {
      x,
      y: ctx.y,
      size,
  ...
  for (const l of facture.lignes) {
    ...
    const rowY = ctx.y + 14;
    ctx.page.drawText(sanitizePdfText(String(l.quantite)), { x: 300, y: rowY, size: 9, font });
    ctx.page.drawText(sanitizePdfText(formatDzdPdf(l.prixUnitaire)), { x: 340, y: rowY, size: 9, font });
    ctx.page.drawText(sanitizePdfText(String(l.tauxTva)), { x: 400, y: rowY, size: 9, font });
    ctx.page.drawText(sanitizePdfText(formatDzdPdf(l.montantTtc)), { x: 460, y: rowY, size: 9, font });
    ctx.y -= 4;
  }
