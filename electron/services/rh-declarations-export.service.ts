...
export async function exportDasAnnuelle(actorUserId: number, annee: number): Promise<RhExportResult> {
  ...
  const lines = [
    ...
    csvLine(['NIN', 'NSS', 'Matricule', 'Nom', 'Prénom', 'Brut annuel', 'Primes', 'CNAS salarié', 'CNAS patronale', 'IRG annuel', 'Net annuel', 'Nb bulletins']),
  ];

  for (const r of rows) {
    const brut = Number(r.brut_annuel) + Number(r.primes_annuelles);
    const paie = calculatePaieDz(brut, 0);
...
export async function exportCnasMensuelle(actorUserId: number, periode: string): Promise<RhExportResult> {
  ...
  const lines = [
    ...
    csvLine(['NSS', 'Nom', 'Prénom', 'Brut', 'CNAS salarié 9%', 'CNAS patronale 26%']),
  ];
  ...
  for (const r of rows) {
    ...
    lines.push(csvLine([String(r.nss ?? ''), String(r.nom), String(r.prenom), brut.toFixed(2), paie.cotisationSalarie.toFixed(2), paie.cotisationPatronale.toFixed(2)]));
  }
  lines.push(csvLine([]));
